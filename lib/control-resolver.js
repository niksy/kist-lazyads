			...
			triggerPromise
				.then(( isControlTriggered ) => {
					if ( isLoaded && isControlTriggered ) {
						...
						if ( typeof loadPromise === 'undefined' ) {
							loadPromise = control.loadResults[id] = control.afterZoneLoad({ element, id });
						}
						return loadPromise;
					}
				...
				.then(( loadResult ) => {
					if ( shouldShow ) {
						...
						if ( isVisible ) {
							control.onZoneShow({ element, id, isContentEmpty, loadResult });
						}
					} else {
						control.onZoneHide({ element, id, isContentEmpty, loadResult });
					}
					return control;
				})
				.catch(( error ) => {
	...
	destroy ( zones ) {
		...
		this.controls.forEach(( control ) => {
			...
			zones.forEach(({ element, id, isContentEmpty }) => {
				...
				control.destroy({ element, id, isContentEmpty, loadResult });
			});
